...
def load_market_data():
    ...
    # Volume surge: vol_ma5 > vol_ma60 * 1.2
    market['volume_surge'] = (
        market['vol_ma5'] > market['vol_ma60'] * 1.2
    )
...
def jit_backtest_core(
    prices_arr, daily_returns_arr, weak_market_arr, volume_surge_arr, ma60_arr,
    ...
    def compute_v2_weights(t, is_weak, is_aggressive, is_surge):
        ...
        if is_weak:
            ...
            if is_surge and not is_weak:
                scale = max(scale, 2.0) # increase scale during volume surge
    ...
    if override_weights_arr is not None:
        ...
        current_weights = compute_v2_weights(0, bool(weak_market_arr[0]), False, bool(volume_surge_arr[0]))
    else:
    ...
    for t in range(1, n_days):
        ...
        if t == trade_start_idx:
            ...
            if override_weights_arr is not None:
                ...
                current_weights = compute_v2_weights(t, bool(weak_market_arr[t]), False, bool(volume_surge_arr[t]))
            holdings = nav * current_weights
        ...
        if override_weights_arr is not None:
            ...
            target_weights = compute_v2_weights(t, bool(weak_market_arr[t]), is_aggressive, bool(volume_surge_arr[t]))
...
def run_backtest(prices, market_data, ma60_arr, base_weights_df,
    ...
    if 'volume_surge' in market_data.columns:
        volume_surge_arr = market_data['volume_surge'].reindex(prices.index).fillna(False).values.astype(np.bool_)
    else:
        volume_surge_arr = np.zeros(len(prices), dtype=np.bool_)
