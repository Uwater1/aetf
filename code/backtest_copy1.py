...
def load_market_data():
    ...
    # Extreme weak market ratio
    # Calculate the ratio: csi300_close / csi300_ema60
    # If ratio is 0.95, cash target is 0.
    # If ratio <= 0.90, cash target is 1.0 (100% cash).
    # Linear scale in between.
    ratio = market['csi300_close'] / market['csi300_ema60']
    market['extreme_weak_market_cash_ratio'] = 0.0

    # Scale from 0 to 1 between 0.95 and 0.90
    mask = ratio < 0.95
    market.loc[mask, 'extreme_weak_market_cash_ratio'] = (0.95 - ratio[mask]) / (0.95 - 0.90)
    market['extreme_weak_market_cash_ratio'] = market['extreme_weak_market_cash_ratio'].clip(0, 1.0)
...
def jit_backtest_core(
    prices_arr, daily_returns_arr, weak_market_arr, extreme_weak_market_cash_ratio_arr, ma60_arr,
    ...
    for t in range(1, n_days):
        ...
        if t == trade_start_idx:
            ...
            target_cash = float(extreme_weak_market_cash_ratio_arr[t]) if use_regime else 0.0
...
def run_backtest(prices, market_data, ma60_arr, base_weights_df,
    ...
    if 'extreme_weak_market_cash_ratio' in market_data.columns:
        extreme_weak_market_cash_ratio_arr = market_data['extreme_weak_market_cash_ratio'].reindex(prices.index).fillna(0.0).values.astype(np.float64)
    else:
        extreme_weak_market_cash_ratio_arr = np.zeros(len(prices), dtype=np.float64)
