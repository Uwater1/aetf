...
def load_market_data():
    ...
    market['strong_market'] = (
        market['csi300_close'] > market['csi300_ema60'] * 1.05
    )
    return market
...
def jit_backtest_core(
    prices_arr, daily_returns_arr, weak_market_arr, extreme_weak_market_arr, strong_market_arr, ma60_arr,
    ...
    def compute_v2_weights(t, is_weak, is_aggressive, is_strong):
    ...
    if override_weights_arr is not None:
        ...
        current_weights = compute_v2_weights(0, bool(weak_market_arr[0]), False, bool(strong_market_arr[0]))
    else:
    ...
    for t in range(1, n_days):
        ...
        if t == trade_start_idx:
            ...
            if override_weights_arr is not None:
                ...
                current_weights = compute_v2_weights(t, bool(weak_market_arr[t]), False, bool(strong_market_arr[t]))
        ...
        if override_weights_arr is not None:
            ...
            target_weights = compute_v2_weights(t, bool(weak_market_arr[t]), is_aggressive, bool(strong_market_arr[t]))
...
def run_backtest(prices, market_data, ma60_arr, base_weights_df,
    ...
    if 'strong_market' in market_data.columns:
        strong_market_arr = market_data['strong_market'].reindex(prices.index).fillna(False).values.astype(np.bool_)
    else:
        strong_market_arr = np.zeros(len(prices), dtype=np.bool_)
